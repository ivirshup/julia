...
function typeinf_ext(linfo, atypes::ANY)
    ...
    inference_stack = EmptyCallStack()
    (newlinfo,ty) = typeinf(linfo, atypes, svec(), true)
...
function typeinf(linfo::LambdaInfo, atypes::ANY, sparams::SimpleVector, needtree=false)
        ...
        for i = 1:3:length(tfarr)
            if typeseq(tfarr[i],atypes)
                ...
                if isa(code,Type)
                    ...
                    return code  # else code is a tuple (linfo, type)
                end
            end
        end
    end
    ...
    (newcode, result, rec) = typeinf_uncached(linfo, atypes, sparams, curtype, true)
    if newcode === nothing
    ...
    if !redo
        ...
        tfarr[idx+1] = rec ? result : (newcode,result)
        tfarr[idx+2] = rec
    else
        def.tfunc[tfunc_idx] = rec ? result : (newcode,result)
...
typeinf_uncached(linfo, atypes::ANY, sparams::ANY; optimize=true) =
    typeinf_uncached(linfo, atypes, sparams, Bottom, optimize)
...
function typeinf_uncached(linfo::LambdaInfo, atypes::ANY, sparams::SimpleVector, curtype, optimize)
    def = linfo.def
    ast0 = def.ast
    #if dbg
